...
// - JS 클래스의 생성자는 한 개만 존재할 수 있다.
    ...
    // 프로토타입 메서드
    // - 클래스 내부에 정의한 메서드는
    // 나동으로 해당 클래스의 프로토타입의 프로퍼티로 추가된다.
    introduce() {
...
console.log(typeof student); // object // 생성된 객체 -> object
console.log(typeof Student); // function // class -> function === 생성자 함수
...
// 클래스 문법과 유사하게 가능하는 것처럼 보이는 생성자 함수도 가능
...
teacher.introduce(); // 안녕하세요 저는 1반 학생 유관순입니다.
...
for (let method in student) {
    console.log(method) // 클래스 / name, group
}
console.log("-----------------")
for (let method in teacher) {
    console.log(method) // 생성자 / group, name, introduce
}
